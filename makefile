...
build-frontend:
	...
	@DISABLE_ESLINT_PLUGIN='true' VITE_REACT_APP_VERSION=$$(cat VERSION) bun run --cwd $(FRONTEND_DIR) build
...
dev-web:
	@echo "Starting frontend dev server: http://localhost:$(DEV_FRONTEND_PORT)"
	@bun install --cwd ./web
	@bun run --cwd $(FRONTEND_DIR) dev -- --host 0.0.0.0 --port $(DEV_FRONTEND_PORT)
